...
public final class IfThenElse extends AbstractOperator {
    ...
    private Sort getCommonSuperSort(Sort s1, Sort s2) {
        if (s1 == SmartMLDLTheory.FORMULA) {
            assert s2 == SmartMLDLTheory.FORMULA : "Sorts FORMULA and " + s2 + " are incompatible.";
            ...
            Sort result = SmartMLDLTheory.ANY;
            ...
            for (final Sort sort1 : set1) {
                if (set2.contains(sort1)) {
                    if (result == SmartMLDLTheory.ANY) {
                        result = sort1;
                    } else {
                        // not uniquely determinable
                        return SmartMLDLTheory.ANY;
                    }
                }
            }
    ...
    public <T extends org.key_project.logic.Term> void validTopLevelException(T term)
        ...
        if (!(s0 == SmartMLDLTheory.FORMULA
                && (s1 == SmartMLDLTheory.FORMULA) == (s2 == SmartMLDLTheory.FORMULA)
                && s1 != SmartMLDLTheory.UPDATE && s2 != SmartMLDLTheory.UPDATE)) {
