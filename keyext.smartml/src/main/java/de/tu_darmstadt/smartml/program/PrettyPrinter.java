...
public final class PrettyPrinter {
    ...
    private String print(Object n, int d) {
        ...
        if (n instanceof Program p) {
            ...
            for (var dec : p.decls())
                sb.append(i(d + 1)).append(print(dec, d + 1)).append("\n");
        ...
        if (n instanceof ContractDecl c) {
            ...
            if (!state.isEmpty()) {
                ...
                for (var s : state)
                    sb.append(i(d + 2)).append(print(s, d + 2)).append("\n");
            }
            ...
            if (!members.isEmpty()) {
                ...
                for (var m : members)
                    sb.append(i(d + 2)).append(print(m, d + 2)).append("\n");
            } else {
                var children = reflectChildren(c);
                if (!children.isEmpty()) {
                    sb.append(i(d + 1)).append("members:\n");
                    for (var ch : children)
                        sb.append(i(d + 2)).append(print(ch, d + 2)).append("\n");
                }
            }
            ...
            if (!invs.isEmpty()) {
                ...
                for (var inv : invs)
                    sb.append(i(d + 2)).append(print(inv, d + 2)).append("\n");
            }
            return sb.toString().trim();
        }
        ...
        if (n instanceof FunctionDecl fd) {
            ...
            if (!params.isEmpty()) {
                ...
                for (var p : params)
                    sb.append(i(d + 2)).append(print(p, d + 2)).append("\n");
            }
            ...
            if (ret != null) {
                ...
                if (ret instanceof String s)
                    ...
                    sb.append(print(ret, d + 1)).append("\n");
            }
        ...
        if (n instanceof InterfaceDecl i0)
            return "InterfaceDecl " + i0.name();
        if (n instanceof DatatypeDecl dt) {
            ...
            if (!kids.isEmpty()) {
                sb.append("\n");
                for (var ch : kids)
                    sb.append(i(d + 1)).append(print(ch, d + 1)).append("\n");
                return sb.toString().trim();
            }
            return sb.toString();
        }
        if (n instanceof ResourceDecl r)
            return "ResourceDecl " + r.name();
        if (n instanceof FieldDecl fd)
            return "Field " + fd.type() + " " + fd.name();
        if (n instanceof FunctionDecl fd) {
            var sb = new StringBuilder("Function ").append(fd.name()).append("\n");
            var params = list(fd, "params", "parameters", "args");
            if (!params.isEmpty()) {
            ...
            if (ret != null)
                sb.append(i(d + 1)).append("ret: ").append(print(ret, d + 1)).append("\n");
        ...
        if (n instanceof Block b) {
            ...
            for (var s : b.statements())
                sb.append(i(d + 1)).append(print(s, d + 1)).append("\n");
            return sb.toString().trim();
        }
        if (n instanceof If x) {
            var sb = new StringBuilder("If\n");
            sb.append(i(d + 1)).append("cond:\n").append(i(d + 2)).append(print(x.cond(), d + 2))
                    .append("\n");
            sb.append(i(d + 1)).append("then:\n").append(i(d + 2))
                    .append(print(x.thenBranch(), d + 2)).append("\n");
            sb.append(i(d + 1)).append("else:\n").append(i(d + 2))
                    .append(print(x.elseBranch(), d + 2));
            return sb.toString();
        }
        ...
        if (n instanceof VarTarget vt) {
            ...
            String s = "VarTarget";
            if (Boolean.TRUE.equals(storage))
                s += " [storage]";
            if (tpe != null)
                s += " : " + print(tpe, d);
            if (target != null)
        ...
        if (n instanceof Var v)
            return "Var " + v.name();
        if (n instanceof IntLit i1)
            return "Int " + i1.value();
        if (n instanceof StringLit s)
            return "String \"" + s.value() + "\"";
        if (n instanceof BoolLit b1)
            return "Bool " + b1.value();
        if (n instanceof AddressLit a)
            return "Address " + a.value();
        if (n instanceof UnaryNeg u)
            return "UnaryNeg\n" + i(d + 1) + print(u.expr(), d + 1);
        if (n instanceof UnaryNot u)
            return "UnaryNot\n" + i(d + 1) + print(u.expr(), d + 1);
        ...
        if (n instanceof QualifiedAccess qa) {
            return "QAccess " + qa.base()
                + (qa.path().isEmpty() ? "" : "." + String.join(".", qa.path()));
        }
