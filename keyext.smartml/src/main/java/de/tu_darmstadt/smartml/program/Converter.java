...
public final class Converter {
    ...
    private final Map<Object, ProgramVariable> local2pv = new HashMap<>();
    ...
    public Program convertProgram(SmartMLParser.ProgramContext p) {
        ...
        for (SmartMLParser.DatatypeDecContext d : p.datatypeDec())  decls.add(convertDatatype(d));
        for (SmartMLParser.ExceptionDecContext e : p.exceptionDec()) decls.add(convertException(e));
        for (SmartMLParser.ResourceDecContext r : p.resourceDec()) decls.add(convertResource(r));
        for (SmartMLParser.InterfaceDecContext i : p.interfaceDec()) decls.add(convertInterface(i));
        for (SmartMLParser.ContractDecContext c : p.contractDec())  decls.add(convertContract(c));
    ...
    private Decl convertContract(SmartMLParser.ContractDecContext c) {
        ...
        var state = new ArrayList<SmartMLProgramElement>();
        var members = new ArrayList<SmartMLProgramElement>();
        var invariants = java.util.List.<SmartMLProgramElement>of();

        var b = c.body();

        for (var fld : b.field()) {
            String t = fld.type().getText();
            String n = fld.id().getText();
            state.add(new FieldDecl(t, n));
        }

        if (b.constructor() != null) {
            var ctor = b.constructor();
            var params = new ArrayList<SmartMLProgramElement>();
            if (ctor.varParams != null) {
                for (var v : ctor.varParams) params.add(convertVarTarget(v));
            }
            Block body = convertConstructorBody(ctor);
            members.add(new ConstructorDecl(params, body));
        }

        for (var f : b.function()) {
            members.add(convertFunction(f));
        }

        return new ContractDecl(name, state, members, invariants);
    }


    private FunctionDecl convertFunction(SmartMLParser.FunctionContext f) {
        var sig = f.functionDec();

        String fname = sig.id().getText();

        var params = new ArrayList<SmartMLProgramElement>();
        if (sig.vardec() != null) {
            for (var v : sig.vardec()) params.add(convertVarTarget(v));
        }

        String retType = null;
        if (sig.returnType() != null) {
            var rt = sig.returnType();
            if (rt.INT() != null)     retType = "int";
            else if (rt.BOOL() != null)   retType = "bool";
            else if (rt.STRING() != null) retType = "string";
            else                          retType = rt.getText();
        }

        Block body = convertBlock(f.statBlock());
        return new FunctionDecl(fname, params, retType, body);
    }
    ...
    private Block convertConstructorBody(SmartMLParser.ConstructorContext ctor) {
        ...
        for (SmartMLParser.AssignContext a : ctor.assign()) stmts.add(convertAssignStmt(a));
    ...
    private Block convertBlock(SmartMLParser.StatBlockContext b) {
        List<Stmt> stmts = new ArrayList<>();
        for (SmartMLParser.StatementContext s : b.statement()) stmts.add(convertStatement(s));
        return new Block(stmts);
    }

    private Stmt convertStatement(SmartMLParser.StatementContext s) {
        if (s.ifStatement()!=null) return convertIf(s.ifStatement());
        if (s.exprStat()!=null)    return new ExpressionStatement(convertExpr(s.exprStat().expr()));
        if (s.loop()!=null)        return new While(convertExpr(s.loop().expr()), convertBlock(s.loop().statBlock()));
        if (s.assign()!=null)      return convertAssignStmt(s.assign());
        if (s.funCall()!=null)     return new CallStmt(convertFunCallAsExpr(s.funCall()));
        if (s.assertError()!=null) return new AssertError(convertExpr(s.assertError().expr()));
        ...
        if (s.returnStat()!=null)  return new Return(convertExpr(s.returnStat().expr()));
        if (s.tryStatement()!=null){
            var tr = s.tryStatement();
        ...
        if (s.statBlock()!=null)   return convertBlock(s.statBlock());
        return new EmptyStatement();
    }

    private If convertIf(SmartMLParser.IfStatementContext i) {
        Expr cond  = convertExpr(i.cond);
        Stmt thenB = convertBlock(i.block);
        Stmt elseB = (i.elseBlock!=null) ? convertBlock(i.elseBlock) : new EmptyStatement();
    ...
    private Assign convertAssignStmt(SmartMLParser.AssignContext a) {
        ...
        Expr rhs = (a.expr()!=null) ? convertExpr(a.expr()) : convertFunCallAsExpr(a.funCall());
    ...
    private VarTarget convertVarTarget(SmartMLParser.VardecContext v) {
        ...
        return new VarTarget(null, storage, target);
    }

    /* ===================== Calls ===================== */

    private Expr convertFunCallAsExpr(SmartMLParser.FunCallContext c) {
        if (c.internalCall()!=null) return convertInternalCall(c.internalCall());
        if (c.externalCall()!=null) return convertExternalCall(c.externalCall());
        if (c.adtCall()!=null)      return convertAdtCall(c.adtCall());
        return new Var(c.getText());
    }

    private QualifiedAccess convertInternalCall(SmartMLParser.InternalCallContext ic) {
        String base = ic.idName.getText();
    ...
    private QualifiedAccess convertQualifiedAccess(SmartMLParser.QualifiedAccessContext q) {
        ...
        for (int i = 1; i < ids.size(); i++) rest.add(ids.get(i).getText());
    ...
    private Expr convertValue(SmartMLParser.ValueContext v) {
        if (v.INTEGER()!=null)   return new IntLit(parseIntToken(v.INTEGER().getSymbol()));
        if (v.string()!=null)    return convertString(v.string());
        if (v.bool()!=null)      return convertBool(v.bool());
        if (v.address()!=null)   return new AddressLit(v.address().getText());
        if (v.LPAR()!=null)      return convertExpr(v.expr());
        if (v.qualifiedAccess()!=null) return convertQualifiedAccess(v.qualifiedAccess());
        if (v.id()!=null)        return new Var(v.id().getText());
        if (v.getChildCount()==2 && v.value()!=null) {
            String op = v.getChild(0).getText();
            Expr inner = convertValue(v.value());
            if ("-".equals(op)) return new UnaryNeg(inner);
            if ("!".equals(op)) return new UnaryNot(inner);
        }
    ...
    private Expr convertString(SmartMLParser.StringContext s) {
        ...
        String unquoted = raw.length()>=2 && raw.charAt(0)=='"' && raw.charAt(raw.length()-1)=='"'
    ...
    private List<Expr> convertParams(SmartMLParser.ParamsContext ps) {
        ...
        for (int i = 1; i < ps.expr().size(); i++) out.add(convertExpr(ps.expr(i)));
        return out;
    }

    private Binary mapBinaryByToken(int tokenType, Expr l, Expr r) {
        switch (tokenType) {
            case SmartMLParser.PLUS:  return new Binary(l, BinOp.ADD, r);
            case SmartMLParser.MINUS: return new Binary(l, BinOp.SUB, r);
            case SmartMLParser.OR:
            case SmartMLParser.ASM:   return new Binary(l, BinOp.OR, r);
            case SmartMLParser.TIMES: return new Binary(l, BinOp.MUL, r);
            case SmartMLParser.DIV:   return new Binary(l, BinOp.DIV, r);
            case SmartMLParser.AND:   return new Binary(l, BinOp.AND, r);
            case SmartMLParser.EQ:    return new Binary(l, BinOp.EQ, r);
            case SmartMLParser.LE:    return new Binary(l, BinOp.LT, r);
            case SmartMLParser.GE:    return new Binary(l, BinOp.GT, r);
            case SmartMLParser.LEQ:   return new Binary(l, BinOp.LE, r);
            case SmartMLParser.GEQ:   return new Binary(l, BinOp.GE, r);
            case SmartMLParser.NEQ:   return new Binary(l, BinOp.NE, r);
        }
    ...
    private void declareLocal(String name, Object declNode) {
        locals.put(name, declNode);
        var ns = services.getNamespaces().programVariables();
        ProgramVariable pv = ns.lookup(new Name(name));
        if (pv != null) local2pv.put(declNode, pv);
    }

    /* ===================== Utilities ===================== */

    private static int parseIntToken(Token t) { return Integer.parseInt(t.getText()); }
}
