...
import de.tu_darmstadt.smartml.calculus.proof.naming.VariableNameProposer;
import de.tu_darmstadt.smartml.calculus.rules.matching.inst.GenericSortCondition;
import de.tu_darmstadt.smartml.calculus.rules.matching.inst.GenericSortException;
...
import de.tu_darmstadt.smartml.calculus.rules.sv.VariableSV;
import de.tu_darmstadt.smartml.calculus.rules.sv.sort.ProgramSVSort;
...
import de.tu_darmstadt.smartml.strategy.RuleAppContainer;
...
public abstract class TacletApp implements RuleApp {
    ...
    public boolean isExecutable() {
        ...
        if (taclet instanceof SMLRewriteTaclet rwt) {
    ...
    public PosTacletApp setPosInOccurrence(PosInOccurrence pos, Services services) {
        if (taclet() instanceof SMLNoFindTaclet) {
            throw new IllegalStateException("Cannot add position to an taclet" + " without find");
        }
        return PosTacletApp.createPosTacletApp((SMLFindTaclet) taclet(), instantiations(),
            ...
            if (nvc != null) {
                ...
                if (o instanceof SchemaVariable peerSV) {
                    ...
                    if (peerInst instanceof Type smlType) {
                        krt = services.getSmartMLInfo().getKeYSmartMLType(smlType.toString());
                    } else {
                        Expr peerInstExpr;
                        /*
                        ...
                        krt = services.getSmartMLInfo()
                                .getKeYSmartMLType(peerInstExpr.type(services).toString());
                    }
                } else {
